...
public class CarInstrumentClusterManager implements CarManagerBase {
    ...
    public static final int ID_IP_SIDE_CHANGE_REQUEST_SET = 0x21401900;
    public static final int ID_IP_MID_CHANGE_REQUEST_SET = 0x21401901;
    public static final int ID_PSC_MODE_SELECT_SET = 0x21401902;
    public static final int ID_IP_BACK_LAMP_LEVEL_QEQUEST_SET = 0x21401903;

    @IntDef({
        ID_IP_SIDE_CHANGE_REQUEST_SET,
        ID_IP_MID_CHANGE_REQUEST_SET,
        ID_PSC_MODE_SELECT_SET,
        ID_IP_BACK_LAMP_LEVEL_QEQUEST_SET,
    })
    ...
    @Retention(RetentionPolicy.SOURCE)
    public @interface MeterSwitchScreenTheme{
        int ThemeNULL = 0;
        int ThemeOne = 0x01;
        int ThemeSecond = 0x02;
        int ThemeThird = 0x03;
        int ThemeMidChange  = 0x04;
    }
    ...
    public void turnPageSide() throws CarNotConnectedException {
        try {
            mService.turnPageSide();
        } catch (RemoteException e) {
            Log.e(TAG, "turnPageSide failed with " + e.toString(), e);
            throw new CarNotConnectedException(e);
        }
    }
    public void turnPageMid(@MeterSwitchScreenTheme int theme)throws CarNotConnectedException {
        try {
            mService.turnPageMid(theme);
        } catch (RemoteException e) {
            Log.e(TAG, "turnPageMid failed with " + e.toString(), e);
            throw new CarNotConnectedException(e);
        }
    }
    // public void setPSCModeSelect(int mode) throws CarNotConnectedException {
    //     try {
    //         mService.setPSCModeSelect(mode);
    //     } catch (RemoteException e) {
    //         Log.e(TAG, "setPSCModeSelect failed with " + e.toString(), e);
    //         throw new CarNotConnectedException(e);
    //     }
    // }
    public void setBackLampBacklight (int level)throws CarNotConnectedException {
        try {
            mService.setBackLampBacklight(level);
        } catch (RemoteException e) {
            Log.e(TAG, "setBackLampBacklight failed with " + e.toString(), e);
            throw new CarNotConnectedException(e);
        }
    }
    /**
