...
public class CarPropertyManager implements CarManagerBase {
                ...
                if (listeners != null) {
                    switch (event.getEventType()) {
                        case CarPropertyEvent.PROPERTY_EVENT_PROPERTY_CHANGE:
                            Log.d("CarPropertyManager", "handleEvent .");
    ...
    public boolean registerListener(CarPropertyEventListener listener, int propertyId, float rate)
        ...
        synchronized (mActivePropertyListener) {
            ...
            if(propertyId == 390071304){
                Log.d(mTag, "carManager trace..."+ Log.getStackTraceString(new Throwable()));
            }
    ...
    private class CarPropertyListeners extends CarRatedFloatListeners<CarPropertyEventListener> {
        ...
        void onPropertyChanged(final CarPropertyEvent event) {
            ...
            Log.d(mTag, "entring CarPropertyManager.java->onPropertyChanged .");
    ...
    /***************************************** aiways add ***************************/
        /** Set CarProperty */
    public void setProperty(int propId, int area)
            throws CarNotConnectedException {
        if (mDbg) {
            Log.d(mTag, "setProperty, propId: 0x" + toHexString(propId)
                    + ", area: 0x" + toHexString(area));
        }
        try {
            mService.setProperty(new CarPropertyValue<>(propId, area,-1));
        } catch (RemoteException e) {
            Log.e(mTag, "setProperty failed with " + e.toString(), e);
            throw new CarNotConnectedException(e);
        }
    ...
    /** Set int[] value of property*/
    public void setIntArrayProperty(int prop, int area, Integer[] val) throws CarNotConnectedException {
        setProperty(Integer[].class, prop, area, val);
    }

    /**cabin need**/
    public float[] getFloatArrayProperty(int prop, int area) throws CarNotConnectedException {
        CarPropertyValue<Float[]> carProp = getProperty(Float[].class, prop, area);
        return carProp != null ? toFloatArray(carProp.getValue()) : new float[0];
    }
