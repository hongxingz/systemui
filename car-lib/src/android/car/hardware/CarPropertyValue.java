...
public class CarPropertyValue<T> implements Parcelable {
    ...
    public CarPropertyValue(Parcel in) {
        ...
        if (String.class.equals(valueClass)) {
            mValue = (T) in.readString();
        } else if (byte[].class.equals(valueClass)) {
            int size = in.readInt();
            byte[] tempValue = new byte[size];
            in.readByteArray(tempValue);
        } else if (Integer[].class.equals(valueClass)) {
            int len = in.readInt();
            Integer[] tempArray = new Integer[len];
            for (int i=0;i<len;i++) {
                tempArray[i] = in.readInt();
            }
            mValue = (T)tempArray;
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        if (String.class.equals(valueClass)) {
            ...
            byte[] tempValue = (byte[]) mValue;
            dest.writeInt(tempValue.length);
            dest.writeByteArray(tempValue);
        } else if (Integer[].class.equals(valueClass)) {
            Integer[] tempArray = (Integer[]) mValue;
            dest.writeInt(tempArray.length);
            for (Integer value : tempArray) {
                dest.writeInt(value);
            }
        } else if (Float[].class.equals(valueClass)) {
            Float[] tempArray = (Float[]) mValue;
            dest.writeInt(tempArray.length);
